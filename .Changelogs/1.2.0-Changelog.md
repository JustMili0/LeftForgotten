...
### ***NEW CONTENT***
- On-ground items now have the same old billboard rendering as in Minecraft Alpha
- Made the Chest, Crafting Table and Furnace also remodel in GUI

### ***CHANGES***
- Updated textures for Brittle Bedrock and Feature Void
- Going back and forth between `left_forgotten:alpha_minecraft` and any other dimension no longer resets player's hunger, it just pauses as it is and resumes when you're out
- Players using Better Combat are no longer granted an attack speed boost in `left_forgotten:alpha_minecraft`
- Mount health is no longer hidden when player is in creative, matching vanilla
- Feature Void now renders similarly to the Barrier Block
- All blocks and items added by the mod now have matching tags with their vanilla counterparts (Including vanilla, `c` or "common" tags, neo/forge-specific and fabric-specific tags)
- Cobblestone and Stone farms of the old variants as well as obtaining the old Obsidian variant is now possible
...
- Fixed version overlay not hiding when F3 is on
- Improved compatibility with Nostalgic Tweaks
